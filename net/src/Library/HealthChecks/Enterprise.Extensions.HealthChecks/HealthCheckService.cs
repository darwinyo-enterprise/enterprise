    ...
    public class HealthCheckService : IHealthCheckService
    {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            using (var scope = GetServiceScope())
            {
                ...
                var groupTasks = _groups.Select(group =>
                    new {Group = group, Task = RunGroupAsync(scopeServiceProvider, group, cancellationToken)}).ToList();
        ...
        {
            return _builder.ChecksByName.Values.ToList().AsReadOnly();
        }

        public CachedHealthCheck GetCheck(string checkName)
        {
            return _builder.ChecksByName[checkName];
        }

        public HealthCheckGroup GetGroup(string groupName)
        {
            return _builder.Groups[groupName];
        }

        public IReadOnlyList<HealthCheckGroup> GetGroups()
        {
            return _builder.Groups.Values.ToList().AsReadOnly();
        }

        public async ValueTask<IHealthCheckResult> RunCheckAsync(CachedHealthCheck healthCheck,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            using (var scope = GetServiceScope())
            {
        ...
        /// </summary>
        public ValueTask<IHealthCheckResult> RunCheckAsync(IServiceProvider serviceProvider,
            CachedHealthCheck healthCheck, CancellationToken cancellationToken = default(CancellationToken))
        {
        ...
        /// </summary>
        public async Task<CompositeHealthCheckResult> RunGroupAsync(HealthCheckGroup group,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            using (var scope = GetServiceScope())
            {
        ...
        {
            return _serviceScopeFactory == null
                ? new UnscopedServiceProvider(_serviceProvider)
