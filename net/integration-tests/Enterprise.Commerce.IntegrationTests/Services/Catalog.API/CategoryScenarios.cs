...
namespace Enterprise.Commerce.IntegrationTests.Services.Catalog.API
{
    ...
    public class Categoriescenarios : CatalogScenarioBase
    {
        ...
        [Fact]
        [TestPriority(6)]
        public async Task Get_Categories_image_response_file_result()
        {
            using (var server = CreateServer())
            {
                var ctx = server.Host.Services.GetRequiredService<CatalogContext>();
        ...
        [Fact]
        [TestPriority(4)]
        public async Task Get_Categories_response_ok_status_code()
        {
            using (var server = CreateServer())
            {
                var ctx = server.Host.Services.GetRequiredService<CatalogContext>();
        ...
        [Fact]
        [TestPriority(5)]
        public async Task Get_Categories_response_ok_status_code_return_base64_instead_of_http_url()
        {
            using (var server = CreateServer())
            {
                var ctx = server.Host.Services.GetRequiredService<CatalogContext>();
